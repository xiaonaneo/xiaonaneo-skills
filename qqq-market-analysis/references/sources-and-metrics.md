...
- 乖离：`D = QQQ / 周MA200 - 1`。与历史乖离比较时使用截至当时可得的同口径历史序列，避免未来数据偏差。D<P80 原则上持有；P80–P90 观察并准备兑现；D≥P90 积极分批卖出；D≥P95 进一步提高卖出力度。没有足够样本时只报告当前百分比，不声称处于历史极端。
...
- 压力速度：至少比较最近一次与此前同频观察值。统一为 `ΔStress > 0` 表示恶化、`ΔStress < 0` 表示改善；`Δ²Stress > 0` 表示恶化速度上升。若原始指标方向相反，先转换符号，并给出时间间隔和变化幅度；优先用多个压力维度确认，不能由单日 VIX 跳升单独触发操作。建仓阶段的压力恶化只降低买入速度；盈利持仓阶段的压力恶化且加速才触发提前减仓评估。
...
- 单一指标只能触发候选、预警或既定的分层动作，不能省略其他两个维度；三者越一致，行动速度或力度越高。
