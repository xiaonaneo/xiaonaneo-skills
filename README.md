...
- 严格分离数据职责：Yahoo adjusted close 只用于 QQQ/TQQQ 价格和均线，Yahoo `DX-Y.NYB` 只用于直接 ICE DXY，FRED/Cboe 用于金融压力，Nasdaq 官方资料用于盈利，forward P/E 使用单一可导出的 Trendonify 月度序列，原油只使用 `NYMEX:CL1!` 连续近月合约；
