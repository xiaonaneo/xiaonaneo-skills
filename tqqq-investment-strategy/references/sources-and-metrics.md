...
| Forward P/E | 单一可验证供应商（执行前固定） | 整次分析和回测使用同一供应商、同一口径、同一版本；供应商或历史版本无法验证时记为 N/A，不拼接替代 |
...
| 金融压力与信用利差 | [Federal Reserve FRED](https://fred.stlouisfed.org/)、[Chicago Fed NFCI](https://www.chicagofed.org/research/data/nfci/current-data)、[St. Louis Fed STLFSI](https://fred.stlouisfed.org/series/STLFSI4) | FRED 负责压力、流动性、利差和宏观时间序列；记录 series ID、频率、观察日和发布日期 |
...
- Forward P/E：每次执行前锁定一个可验证供应商，当前值、历史序列和 vintage 不得跨供应商拼接；必须保存发布日期/时间戳与当时可见版本。无法验证时，估值判定为 N/A；Trailing/harmonic P/E 不能替代 forward P/E。
