...
3. 金融压力：HY OAS、NFCI、VIX 的当前水平、历史百分位、13 周变化方向、改善/恶化速度及速度变化。
...
分别报告 HY OAS、NFCI、VIX，不合并成 C/L/E 或其他投票。三者统一为每周最后一个可用观测，并分别提供：当前值、历史百分位、13 周变化方向、近 4 周改善/恶化速度，以及该速度相对前一四周窗口的变化。

- HY OAS 上升表示信用压力加大，下降表示缓解。
- NFCI 上升表示金融条件收紧，下降表示放松。
- VIX 上升表示隐含波动压力加大，下降表示缓解。

三个指标的历史百分位均按“数值越高、压力越大”解释。13 周方向、速度和速度变化使用各指标自身的同口径序列；缺失比较窗口时，对应字段为 NA，不影响另外两个指标。速度为正表示压力恶化，速度为负表示压力改善；速度变化为正表示恶化加速或改善减速，速度变化为负表示恶化减速或改善加速。
...
面向用户的所有可见内容使用简体中文。只显示精简数据和结论，不列公式、计算过程、抓取流程、来源优先级或内部字段解释。严格使用以下四个标题，顺序不可变；不增加其他标题。
...
**金融风险**
数据：HY OAS、NFCI、VIX 的当前值、历史百分位、13 周变化方向、改善/恶化速度和速度变化。
